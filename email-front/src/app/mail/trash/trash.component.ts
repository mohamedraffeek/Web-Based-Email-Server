...
export class TrashComponent implements OnInit{
  ...
  loadEmails(){
    this.service.getEmails(Master.Username, "Trash").subscribe((mailList: Array<Email>) => {
      this.allEmails = [];
      if(mailList != null){
        mailList.forEach(
          (email: Email) => {
  ...
  restoreEmails(){
    for(let i = 0; i < 14; i++){
      if(this.checkboxes[i].isChecked){
        ...
        console.log(i);
        this.service.restoreEmail((this.allEmails.length - ((this.pageNumber - 1) * 14 + i) - 1)).subscribe(() => {
          this.allEmails = [];
          this.emails = [];
          this.loadEmails();
        });
  ...
  on(mailNumber: number){
    ...
    if(!this.emails[mailNumber].getRead()){
      ...
      this.service.readEmail(this.emails[mailNumber], "Trash").subscribe();
    }
  }
